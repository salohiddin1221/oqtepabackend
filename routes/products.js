...
const {storage , checkFileType} = require('../routes/uploadRouter')
...
route.post('/', upload.single('productImage'), async (req, res) => {
    try {
        ...
        product.ImageUrl = `https://res.cloudinary.com/du6osjcvh/image/upload/oqtepa/${product.productImage}`;
        await product.save();
...
route.put('/:id',upload.single('productImage'),  async (req, res) => {
    if (req.file) {
        var updatedProduct = {
            ...
            ImageUrl: `https://res.cloudinary.com/du6osjcvh/image/upload/oqtepa/${req.file.originalname}`
        }
    } else {
        var updatedProduct = {
            name: req.body.name,
